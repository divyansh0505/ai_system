...
9. [Code Comments Policy](#9-code-comments-policy) — **CODE QUALITY**
10. [Widget / Embeddable Asset Paths](#10-widget--embeddable-asset-paths) — **CRITICAL**
    - 10.1 [Always Import Static Assets via Vite](#101-always-import-static-assets-via-vite)
    - 10.2 [Testing Cross-Origin Asset Resolution](#102-testing-cross-origin-asset-resolution)
...
## 10. Widget / Embeddable Asset Paths

**Impact: CRITICAL**

This project is an embeddable widget loaded via `<script>` tag on third-party sites (e.g., `sprinto.com`). The built `main.js` is served from a CDN (S3/CloudFront), but the host page is on a completely different domain. This means **absolute or hardcoded asset paths will break in production**.

### 10.1 Always Import Static Assets via Vite

**Impact: CRITICAL (images/assets will 404 on third-party sites otherwise)**

Never use hardcoded string paths for images, SVGs, videos, or other static assets. Always use Vite's `import` so the bundler resolves the path relative to the script's origin (`import.meta.url`), not the host page's domain.

**Why this matters:** When the widget is embedded on `sprinto.com` but served from `cdn.interactlabs.ai`, a path like `/images/avatar.png` resolves to `sprinto.com/images/avatar.png` (404) instead of `cdn.interactlabs.ai/images/avatar.png`.

**Incorrect: hardcoded string path**

```typescript
// In a config/constant file
export const AGENT_IMAGE = "/images/maya-image.png";
...
These resolve against the host page's domain, not the CDN where the widget is served from.

**Correct: Vite import**

```typescript
// In a config/constant file
import agentImage from "/images/maya-image.png";
export const AGENT_IMAGE = agentImage;
...
Vite transforms these into `new URL("images/file.png", import.meta.url).href`, which resolves relative to the script's own URL (the CDN), not the host page.

**Rule of thumb:** If it's a path to a file in `public/` or project assets, it must be an `import`, never a string literal.

### 10.2 Testing Cross-Origin Asset Resolution

**Impact: HIGH (catches broken assets before deployment)**

Always test the built widget in a cross-origin setup before deploying. The dev server works fine because everything is same-origin — bugs only surface when the script loads from a different domain than the page.

**Local cross-origin test:**

```bash
# Terminal 1: serve built widget on port 4001 (simulates CDN)
npm run build && npx serve dist -p 4001 --cors

# Terminal 2: serve test page on port 4002 (simulates third-party site)
npx serve example -p 4002

# Open: http://localhost:4002/local-prod.html
```

Check the browser console for 404 errors. Image requests should go to `localhost:4001` (the script origin), not `localhost:4002` (the page origin).
